...
class Checkable(BaseModel):
    ...
    checks: Set[str] = Field(
        default_factory=set,
        description="Scores which have had a critical success this season",
    )
...
class Traits(HasExtras):
    ...
    # Core trait values
    chaste: int = Field(
        ..., description="Attitude towards carnal pleasure and romantic fidelity", gt=0
    )
    energetic: int = Field(
        ..., description="Behaviors related to activity and inactivity", gt=0
    )
    forgiving: int = Field(
        ..., description="Whether revenge is an acceptable pathway to justice", gt=0
    )
    generous: int = Field(
        ..., description="Willingness to share or not share with others", gt=0
    )
    honest: int = Field(
        ...,
        description="How much they are willing to bend truth to their own needs",
        gt=0,
    )
    just: int = Field(
        ..., description="Whether they believe laws govern all people", gt=0
    )
    merciful: int = Field(
        ..., description="An innate sense (or lack) of regard to others", gt=0
    )
    modest: int = Field(
        ...,
        description="How much a person thinks of themselves in comparison to others",
        gt=0,
    )
    prudent: int = Field(
        ..., description="How much a person thinks before acting", gt=0
    )
    spiritual: int = Field(
        ...,
        description="A character's attitude toward and faith in the unseen world.",
        gt=0,
    )
    temperate: int = Field(
        ..., description="A character's appetite for good food and drink.", gt=0
    )
    trusting: int = Field(
        ...,
        description="The level of faith a person places "
        "in the motivations and intentions of others",
        gt=0,
    )
    valorous: int = Field(
        ...,
        description="How brave and audacious a character may be during times of extreme duress",
        gt=0,
    )
...
class Statistics(Checkable):
    ...
    siz: int = Field(
        ...,
        description="Size and weight compared to others. Also the knockdown modifier",
    )
...
class Skills(HasExtras):
    ...
    flirting: int = Field(
        ..., description="Communicating charm and gaining someone's interest"
    )
    folklore: int = Field(
        ...,
        description="Knowledge of the culture, laws, and customs of the common class",
    )
    gaming: int = Field(
        ...,
...
class PendragonCharacter(Character):
    ...
    distinctive_features: List[str] = Field(
        default_factory=list, description="Easy ways of telling them apart visually"
    )
    ...
    def glory_roll(self) -> int:
        return int(round(self.glory / 1000.0, 0))
    ...
    def describe_ability(self, ability_name: str) -> str:
        ...
        for category in [self.skills, self.statistics, self.passions, self.traits]:
            if (
                value := getattr(category, ability_name, None)
            ) is not None and isinstance(value, int):
                return f"TN{value}"
        else:
