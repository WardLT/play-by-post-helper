...
def _get_sheet_class(guild_id: int) -> type[Character]:
    return config.team_options[guild_id].character_sheet
...
def list_available_characters(guild_id: int, user_id: int) -> List[str]:
    ...
    return [
        ...
        if _get_sheet_class(guild_id).from_yaml(s).player == user_id
    ]
...
def load_character(guild_id: int, name: str) -> Tuple[Character, Path]:
    ...
    guild = _get_sheet_class(guild_id)
    return guild.from_yaml(sheet_path), sheet_path.absolute()
