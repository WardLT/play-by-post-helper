...
def attach_commands(
    bot: ModronClient, modules: Sequence[InteractionModule]
) -> NoExitParser:
...
async def handle_generic_command(
    parser: NoExitParser, modules: Sequence[InteractionModule], context: Context, *args
):
    ...
    logger.info(
        f"Received command from user {context.author.name} on {context.channel.name}: {' '.join(args)}"
    )
