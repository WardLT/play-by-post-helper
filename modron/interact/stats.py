...
class StatisticModule(InteractionModule):
    ...
    def register_argparse(self, parser: ArgumentParser):
        ...
        parser.add_argument(
            "--all-players", action="store_true", help="Get actions from all players"
        )
        parser.add_argument(
            "--character",
            "-c",
            type=str,
            default=None,
            help="Name of the character to query",
        )
        parser.add_argument(
            "--reason",
            type=str,
            default=None,
            help="Purpose of the roll (e.g., perception)",
        )
        parser.add_argument("--die", type=str, default="d20", help="Type of the dice")
        parser.add_argument(
            "--channel", type=str, help="Which channel(s) to draw from. Can be a regex"
        )
        parser.add_argument(
            "--no-modifiers",
            action="store_true",
    ...
    async def interact(self, args: Namespace, context: Context):
        ...
        if len(dice_log) > 0:
            dice_log["dice_faces"] = dice_log["dice"].apply(
                lambda x: DiceRoll.make_roll(x).dice
            )
            dice_log = dice_log[dice_log["dice_faces"].apply(lambda x: die_choice in x)]
            logger.info(
                f"Reduced to {len(dice_log)} records that rolled a d{die_choice}"
            )

        if args.reason is not None and len(dice_log) > 0:
            dice_log.query(f'reason=="{args.reason}"', inplace=True)
            logger.info(
                f'Reduced to {len(dice_log)} records with purpose "{args.reason}"'
            )

        if args.no_modifiers and len(dice_log) > 0:
            dice_log.query(
                "not (advantage or disadvantage or reroll_ones)", inplace=True
            )
        ...
        for _, row in dice_log.iterrows():
            rolls.extend(
                [
                    v
                    for f, v in zip(row["dice_faces"], row["dice_values"])
                    if f == die_choice
                ]
            )
        ...
        output = [
            header,
            f"*Last {min(5, len(rolls))} rolls*: {', '.join(map(str, rolls[-5:]))}",
        ]
        if args.reason is None:
            common_rolls = dice_log[~dice_log["reason"].isnull()].reason.value_counts()
            output.append(
                f"*Most common rolls*: {', '.join(common_rolls.iloc[:3].index)}"
            )
