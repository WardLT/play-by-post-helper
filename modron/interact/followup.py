...
class FollowupModule(InteractionModule):
    ...
    def __init__(self):
        ...
        super().__init__(
            "rem",
            "Remind self about a message later",
    ...
    async def interact(self, args: Namespace, context: Context) -> Optional[Task]:
        ...
        if context.author.id in team_options.private_channels:
            reply_channel = utils.get(
                context.guild.channels,
                name=team_options.private_channels[context.author.id],
            )
        logger.info(
            f"Watching for reminders on {context.channel.name},"
            f" reminding user on {reply_channel.name} at {datetime.now() + duration}"
        )

        # Set up a timer to reply when that occurs
        task = asyncio.create_task(
            self.reply_if_needed(
                duration, context.author, context.channel, reply_channel
            )
        )
    ...
    async def reply_if_needed(
        self,
        sleep_time: timedelta,
        user: User,
        watch_channel: TextChannel,
        reply_channel: TextChannel,
    ) -> bool:
        ...
        logger.info(
            f"Awake and looking for messages from {user.display_name} after {start_time}"
        )
