...
class BackupService(BaseService):
    ...
    def get_folder_id(self) -> str:
        ...
        if len(hits) > 1:
            ...
            result = self.gdrive_client.files().create(body=file_metadata).execute()
    ...
    async def backup_messages(self, channel: TextChannel) -> int:
        ...
        logger.info(
            f"Starting to backup chanel {channel} from {self.guild_name} to {output_path}"
        )
        ...
        logger.info(
            f"Starting timestamp {start_time}, which is {datetime.fromtimestamp(start_time)}"
        )

        # Pulling the most recent message
        last_time, _ = await get_last_activity(channel)
        if isclose(
            (last_time - datetime.fromtimestamp(start_time)).total_seconds(), 0.0
        ):
        ...
        with open(output_path, "a") as fp:
            ...
            async for message in channel.history(
                after=after, limit=None, oldest_first=True
            ):
    ...
    async def backup_all_channels(self) -> Dict[str, int]:
        ...
        tasks = dict(
            (c.name, asyncio.create_task(self.backup_messages(c))) for c in to_backup
        )

        # Wait until they all finish
        return dict([(c, await t) for c, t in tasks.items()])
    ...
    async def upload_to_gdrive(self) -> Tuple[int, int]:
        ...
        output = (
            self.gdrive_client.files().get(fileId=config.gdrive_backup_folder).execute()
        )
        assert output.get("mimeType", None) == "application/vnd.google-apps.folder"
        logger.info(
            f'Ready to upload to "{output["name"]}" ({config.gdrive_backup_folder})'
        )
    ...
    async def upload_file(self, file: Union[str, Path]) -> Tuple[bool, int]:
        ...
        result = (
            ...
            .list(
                ...
                pageSize=2,
                fields="files/id,files/md5Checksum,files/size,files/modifiedTime",
            )
            .execute()
        )
        hits = result.get("files", [])
        ...
        if len(hits) > 1:
            ...
            last_uploaded = datetime.strptime(
                hits[0]["modifiedTime"], "%Y-%m-%dT%H:%M:%S.%fZ"
            )
            ...
            async with make_compressed_version(file_path) as to_upload:
                ...
                media = MediaFileUpload(
                    str(to_upload), mimetype="application/jsonlines"
                )
                result = (
                    self.gdrive_client.files()
                    .update(
            ...
            # Upload the file
            async with make_compressed_version(file_path) as to_upload:
                file_metadata = {"name": to_upload.name, "parents": [folder_id]}
                media = MediaFileUpload(
                    str(to_upload), mimetype="application/jsonlines"
                )
                result = (
    ...
    async def run(self):
        ...
        while not self.stop.is_set():
            ...
            logger.info(
                f"Backed up {sum(result.values())} messages in total. From: {', '.join(result.keys())}"
            )
            ...
            if self._creds is not None:
                try:
                    ...
                    self.last_backup_successful = True
                    logger.info(
                        f"Updated {count} files. Uploaded {humanize.naturalsize(data_size, binary=True)}"
                    )
