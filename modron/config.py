...
class TeamConfig(BaseModel):
    ...
    name: str = Field(
        ...,
        description="Name of the Slack team to be used internally by Modron. "
        "Must be unique from the other teams and should be more memorable than "
        "the unique ID generated by Slack.",
    )
    ooc_channel: str = Field(
        "ooc_discussion", description="Name of the general discussion channel"
    )
    system: Literal["dnd5", "pendragon"] = Field(
        "dnd5", description="Which system to use"
    )

    # Private channels
    private_channels: Dict[int, str] = Field(
    ...
    reminder_channel: str = Field(
        "ic_all", description="Channel on which to post reminders"
    )
    watch_channels: List[str] = Field(
        None,
        description="Names of channels to watch for activity. "
        "Can include categories or specific channels",
    )
    allowed_stall_time: timedelta = Field(
        timedelta(days=1),
        description="How long to wait for activity before issuing reminders",
    )
    reminder_window: tuple[time, time] = Field(
    ...
    # Backing up messages
    backup_channels: Optional[List[int]] = Field(
        default_factory=list,
        description="List of channels to back up. Can include category channels",
    )

    # Logging dice rolls
    dice_log: bool = Field(True, description="Whether to log dice rolls for this team")
    dice_tracked_categories: List[int] = Field(
        default_factory=list, description="List of categories of channel to track"
    )
    blind_channel: str = Field(
        "blind_rolls", description="Name of the channel on which to post blind rolls"
    )
    blind_rolls: List[str] = Field(
        default=(), description="Purposes of rolls which are always blind"
    )
    public_channel: Optional[str] = Field(
        default=None, description="If provided, public roll results will be sent here"
    )
    ...
    @property
    def character_sheet(self) -> type[Character]:
        """Character sheet class for this system"""
        if self.system == "dnd5":
            return DnD5Character
        elif self.system == "pendragon":
            return PendragonCharacter
        else:
...
class ModronConfig(BaseModel):
    ...
    state_path: str = Field(
        str((my_path / ".." / "modron_state.yml").absolute()),
        description="Path to the Modron state YAML file",
    )
    dice_log_dir: str = Field(
        "dice-logs",
        description="Path to where the dice logs are stored. One per channel, "
        "labelled with name of team defined in this config file.",
    )
    backup_dir: str = Field(
        "backup",
        description="Path to where to store the backup. Each team will get its own "
        "subdirectory",
    )
    character_dir: str = Field(
        str((my_path / ".." / "characters").absolute()),
    ...
    gdrive_backup_folder: str = Field(
        "1OmFkSgRvBr3JeWCnOWOaHBiEX5T-AR_V",
    ...
    npc_race_weights: List[float] = Field(
        _RACE_TIER_WEIGHTS,
        description="Weights for different probability tiers",
        min_length=4,
        max_length=4,
    )
    npc_race_dist: Dict[str, List[List[str]]] = Field(
        _RACE_DISTRIBUTION,
        description="Common races for different locations. Each location is defined by a list of "
    ...
    npc_relationship_dist: List[Tuple[int, str]] = Field(
        _RELATIONSHIP_DIST,
        description="Distribution of relationship outcomes. Each item in the list is a tuple "
    ...
    npc_age_dist: List[Tuple[int, str]] = Field(
        _AGE_DISTRIBUTION,
        description="Distribution of character ages. Each item in the list is a tuple where "
