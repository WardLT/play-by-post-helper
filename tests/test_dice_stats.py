...
def test_fair():
    m = FairDie(6)
    assert isclose(m.compute_likelihood(1), 1.0 / 6)
    assert np.isclose(m.compute_likelihood([1, 2]), [1.0 / 6] * 2).all()


def test_slanted():
    m = SlantedDie(6, 1)
    assert isclose(m.compute_likelihood(6), 1.0 / 6)
    for w in [0.5, 2.0]:
        m.set_params([w])
...
@mark.parametrize("model", [FairDie(6), SlantedDie(6, 2), ExtremeDie(6, 12)])
...
def test_fit():
    ...
    m = SlantedDie(6)
    fit_model(r, m)
    assert np.isclose(m.get_params(), [1.0]).all()
