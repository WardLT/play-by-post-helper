...
def _measure_probability(
    sides: int, target_val: int, n_trials: int = _default_rolls, **kwargs
) -> float:
    ...
    hits = sum(
        map(lambda x: roll_die(sides, **kwargs)[0] == target_val, range(n_trials))
    )
...
def test_advantage_reroll():
    assert isclose(
        _measure_probability(2, 2, advantage=True, reroll_one=True),
        1 - (0.25 * 0.5),
        abs_tol=1e-3,
    )
...
def test_parser():
    ...
    assert roll.dice == [14, 14, 14, 14, 3]
    assert not roll.advantage
    assert not roll.disadvantage
